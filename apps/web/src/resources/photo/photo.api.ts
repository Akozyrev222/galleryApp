...
import { photoTypes } from 'resources/photo';


export function uploadPhoto<T>() {
    const uploadPhoto = (data: T) => {
        apiService.post('/photo/upload', data)
        console.log(data)
    };

    return useMutation<photoTypes.Photo, unknown, T>(uploadPhoto, {
        onSuccess: (data) => {
